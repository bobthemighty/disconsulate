...
class StubLogger {
  ...
  log(level, msg) {
    console.log(`${new Date().toIsoString()} Consulite ${level} ${msg}`);
  }
}
/* $lab:coverage:on$ */
...
class ServiceDescriptor {
  constructor(
    { service, near = "agent", passing = 1, tags = [], dc, node = {} } = {}
  ) {
  ...
  next() {
    if (this.nodes.length == 0) {
      return Promise.reject(
        new Error(`No nodes found for service '${this.describe()}'`)
      );
    }
...
module.exports = class Disconsulate extends EventEmitter {
  constructor(options = {}) {
    ...
    this.seed = retry.seedDelay || 100;
    this.max = retry.maxDelay || 30000;
    this.retries = retry.maxTries || 20;
    this.logger = options.logger || new StubLogger();
    this.consulAddr = options.consul || process.env.CONSUL_ADDR;
  ...
  async insertToCache(desc, isWatch, waiter) {
    ...
    try {
      ...
      if (!isequal(new Set(nodes), new Set(desc.nodes))) {
        this.logger.info(
          `Received new addresses for service ${desc.describe()}: ${JSON.stringify(
            nodes,
            null,
            2
          )}`
  ...
  async getService(opts) {
    ...
    const desc = new ServiceDescriptor(opts);
    const path = desc.uri;
    ...
    this.logger.debug(
      `${desc.describe()} is not cached, fetching for the first time.`
    );
